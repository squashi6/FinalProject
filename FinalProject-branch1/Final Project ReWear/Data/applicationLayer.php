...
switch($action)
{
    ...
    case "LOADORDERS" : loadOrdersFunction();
                    break;
    case "SAVESTYLE"  : saveStyleFunction();
                    break;
    case "LOADSTYLE"  : loadStyleFunction();
                    break;
    case "CHANGEPROFILE"  : updateProfileFunction();
                    break;                    
...
function saveStyleFunction(){
    $userId = $_POST["userId"];
    $type = $_POST["type"];
    $size = $_POST["size"];
    $colors = $_POST["colors"];
    $brands = $_POST["brands"];
    $result= saveStyle($userId,$type,$size,$colors,$brands);
    if($result["MESSAGE"]=="SUCCESS"){
...
function updateProfileFunction(){
    ...
    $email = $_POST["email"];
    $country = $_POST['country'];
    $result = updateProfile($userId,$uName,$fName,$lName,$email,$country);
    if($result["MESSAGE"]=="SUCCESS")
    echo json_encode($result);
    else{
        genericErrorFunction($result["MESSAGE"]);
